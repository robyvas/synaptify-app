...
import { PlusCircle, Banknote, Type, Tag, Building2, User, Users, Sliders, Check } from 'lucide-react';
...
const IncomeForm: React.FC<IncomeFormProps> = ({ onAdd }) => {
  ...
  useEffect(() => {
    ...
    if (splitType === 'robert') {
      ...
      // Custom: update amounts based on current percentages
    ...
    >
      <div className="flex items-center justify-between pb-4 border-b border-zinc-800/60">
        <h2 className="text-lg font-bold text-zinc-100 flex items-center">
          ...
          Adaugă Încasare Nouă
        </h2>
        <span className="text-xs text-zinc-500 font-medium">Secțiunea Total</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="relative group">
          ...
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              <Type className="h-4 w-4 text-zinc-500 group-focus-within:text-purple-400 transition-colors" />
            </div>
            <input
              type="text"
              placeholder="ex: Proiect Website Clinică / Mentenanță"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl text-zinc-100 placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all text-sm font-medium"
              required
            />
          </div>
        </div>
        ...
        <div className="relative group">
          ...
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              <Banknote className="h-4 w-4 text-zinc-500 group-focus-within:text-emerald-400 transition-colors" />
            </div>
            <input
              type="number"
              placeholder="ex: 3500"
              ...
              min="0"
              step="1"
              className="w-full pl-10 pr-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl text-zinc-100 placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all text-sm font-bold"
              required
            />
          </div>
        </div>
      ...
      <div className="space-y-2.5">
        <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider flex items-center">
          <Tag className="w-3.5 h-3.5 mr-1.5 text-zinc-500" /> Categorii Serviciu
        </label>
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 border flex items-center space-x-1.5 ${
                  isSelected 
                    ? 'bg-purple-600 border-purple-500 text-white shadow-lg shadow-purple-900/30 ring-1 ring-purple-400/40' 
        ...
        {selectedCategories.length === 0 && (
          <p className="text-xs text-amber-500/90 pl-1 font-medium">* Selectează cel puțin o categorie pentru a continua</p>
        )}
      </div>
      ...
      <div className="pt-2">
        <div 
          ...
          className={`cursor-pointer border rounded-2xl p-4 md:p-5 transition-all duration-200 flex items-center justify-between ${
            isCompany 
              ? 'bg-gradient-to-r from-blue-950/40 via-zinc-900 to-indigo-950/40 border-blue-500/40 shadow-lg shadow-blue-950/20' 
        ...
        >
          <div className="flex items-center space-x-3.5">
            <div className={`p-2.5 rounded-xl border transition-colors ${
              isCompany 
                ? 'bg-blue-600 text-white border-blue-400/50 shadow-md shadow-blue-900/40' 
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-sm font-bold text-zinc-100">Încasare Pe Firmă</span>
                {isCompany && (
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Bifează dacă această sumă este facturată/încasată pe societate și trebuie împărțită între asociați.
              </p>
            </div>
          </div>
          ...
          <div className="relative inline-flex items-center cursor-pointer shrink-0 ml-4">
            <div className={`w-12 h-6.5 rounded-full transition-colors duration-300 p-1 flex items-center ${
              isCompany ? 'bg-blue-600 justify-end' : 'bg-zinc-800 justify-start'
            }`}>
              <motion.div 
                layout 
        ...
        <AnimatePresence>
          {isCompany && (
            <motion.div
              ...
              className="overflow-hidden bg-zinc-950/80 border border-blue-500/20 rounded-2xl p-5 md:p-6 space-y-5"
            >
              <div>
                <label className="text-xs font-bold text-blue-400 uppercase tracking-wider block mb-3">
                  Atribuire Încasare Firmă
                </label>
              ...
              <div className="bg-zinc-900/60 p-3.5 rounded-xl border border-zinc-800/80 space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-zinc-400 font-medium">Sumar Împărțire Firmă:</span>
      ...
      <motion.button
        ...
        className={`w-full mt-4 py-3.5 px-6 text-white font-bold rounded-xl shadow-lg transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 ${
          selectedCategories.length === 0 
            ...
            : isCompany
              ? 'bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 shadow-blue-900/30 focus:ring-blue-500'
              : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 shadow-purple-900/30 focus:ring-purple-500'
        }`}
      >
        {isCompany ? 'Înregistrează Venitul (Pe Firmă)' : 'Înregistrează Venitul'}
      </motion.button>
    </motion.form>
  );
};
